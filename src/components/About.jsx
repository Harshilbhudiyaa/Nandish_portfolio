...
const services = [
  {
    ...
    description: "Scroll-stopping launches, product promotions and social systems designed around one sharp commercial idea.",
    image: projects.find((project) => project.id === "jbl-black-friday").image,
  },
  {
    number: "02",
    title: "Photo Manipulation",
    description: "Cinematic composites, product worlds and concept-driven visual effects with convincing light, depth and atmosphere.",
    image: projects.find((project) => project.id === "ember-x").image,
  },
  {
    number: "03",
    title: "Editorial & Posters",
    description: "Expressive typography, strong grids and image-led layouts built to command attention in print and digital formats.",
    image: projects.find((project) => project.id === "fashion-editorial").image,
  },
  {
    number: "04",
    title: "Brand Visual Systems",
    description: "A consistent visual language for products, restaurants and growing businesses across campaigns and content.",
    image: projects.find((project) => project.id === "auraa-herbal").image,
  },
];
...
export default function About() {
  ...
  const orbRotate = useTransform(scrollYProgress, [0, 1], [0, 170]);

  return (
    <section id="about" ref={ref} className="relative overflow-hidden bg-ink py-24 text-white md:py-32 lg:py-40">
      <div className="absolute inset-0 grid-pattern opacity-[0.07]" />
      <motion.div style={{ y: orbY, rotate: orbRotate }} className="pointer-events-none absolute -right-36 top-24 h-[440px] w-[440px] rounded-full border border-white/10">
      ...
      <div className="page-shell relative z-10">
        ...
        <div className="grid gap-16 lg:grid-cols-[0.88fr_1.12fr] lg:gap-20 xl:gap-28">
          ...
          >
            <h2 className="font-display text-[15vw] font-bold leading-[0.79] tracking-[-0.078em] sm:text-[11vw] lg:text-[6.15vw]">
              BUILT WITH<br /><span className="text-acid">INTENT.</span>
            </h2>
            <p className="mt-9 max-w-xl text-lg leading-relaxed text-white/60">
              I am Nandish, a graphic designer focused on bold commercial visuals. My process combines concept, hierarchy, detailed image treatment and disciplined composition so every piece feels clear, premium and memorable.
            </p>
            <a href="#contact" data-cursor="Let's talk" className="group mt-9 inline-flex items-center gap-4 border-b border-white/25 pb-2 font-medium transition-colors hover:border-acid hover:text-acid">
              Discuss a project <ArrowUpRight size={18} className="transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1" />
            </a>

            <div className="relative mt-12 hidden overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] p-3 lg:block">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="aspect-[16/10] w-full overflow-hidden rounded-[20px]"
                >
            ...
            {services.map((service, index) => (
              ...
              >
                <motion.span
                  ...
                  className="absolute inset-x-0 bottom-0 h-px origin-left bg-acid"
                />
        ...
        <div className="mt-24 grid gap-px overflow-hidden rounded-[26px] border border-white/10 bg-white/10 sm:grid-cols-3">
          {[
            ["12", "Selected projects", "Across seven visual disciplines"],
            ["06", "Core capabilities", "From concept through final artwork"],
            ["100%", "Original thinking", "Every visual built for its brief"],
