...
function MagneticButton() {
  ...
  return (
    <motion.a ref={ref} style={{ x, y }} onPointerMove={move} onPointerLeave={() => { rawX.set(0); rawY.set(0); }} href={`mailto:${CONTACT_EMAIL}`} data-cursor="Start a project" className="group relative flex h-40 w-40 shrink-0 items-center justify-center rounded-full bg-ink text-white shadow-[0_30px_80px_rgba(16,16,15,0.24)] transition-colors hover:bg-white hover:text-ink sm:h-48 sm:w-48">
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative overflow-hidden bg-acid py-24 text-ink md:py-32 lg:py-40">
      <div className="absolute inset-0 contact-grid opacity-30" />
      <motion.div animate={{ rotate: 360 }} transition={{ duration: 42, repeat: Infinity, ease: "linear" }} className="pointer-events-none absolute -right-36 -top-36 h-[520px] w-[520px] rounded-full border border-ink/20">
      ...
      <div className="page-shell relative z-10">
        ...
        <motion.h2 initial={{ opacity: 0, y: 38 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }} className="font-display text-[17vw] font-bold leading-[0.76] tracking-[-0.088em] sm:text-[13vw] lg:text-[7.5vw]">
        ...
        <div className="mt-16 grid gap-12 border-t border-ink/20 pt-9 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <p className="max-w-2xl font-display text-3xl font-medium leading-[1.05] tracking-[-0.045em] sm:text-5xl">Have a campaign, poster, product launch or visual idea that needs a stronger frame?</p>
            <p className="mt-7 max-w-xl text-base leading-relaxed text-ink/60">Share the brief, required formats and timeline. I am available for advertising visuals, social campaigns, editorial design, poster systems and photo manipulation.</p>
            <div className="mt-10 flex flex-wrap items-center gap-3">
              ...
              <button type="button" onClick={copyEmail} className="flex h-12 w-12 items-center justify-center rounded-full border border-ink/25 transition-colors hover:bg-ink hover:text-white" aria-label="Copy email address">{copied ? <Check size={17} /> : <Copy size={17} />}</button>
      ...
      <div className="mt-24 overflow-hidden border-y border-ink/20 py-5">
        <div className="animate-marquee-reverse flex w-max items-center gap-10 will-change-transform">
          {[...Array(2)].flatMap((_, loop) => ["ART DIRECTION", "CAMPAIGNS", "EDITORIAL", "COMPOSITING", "POSTERS", "BRAND VISUALS"].map((item) => `${item}-${loop}`)).map((key) => (
            <div key={key} className="flex items-center gap-10"><span className="font-display text-3xl font-bold tracking-[-0.045em] sm:text-5xl">{key.split("-")[0]}</span><span className="text-2xl">✦</span></div>
          ))}
        </div>
      </div>
    </section>
